...
def insert_direct(data_list=None, tablename='r_t_mkmkr_contract', con='oracle://test2:test2@10.21.68.211:1521/hsfa?charset=utf8'):

    tablename = tablename.lower()
    engine = create_engine(con, echo=True) # 生产库
    connection = engine.connect()
    meta = MetaData(bind=engine)
    seqname = tablename + '_id'
    seqname = seqname.upper()
    o_table = Table(tablename,
                    meta,
                    Column('id', Integer, Sequence(seqname), primary_key=True),
...
def pd_insert_db(df, tablename='r_t_mkmkr_contract'):
    data_list = df.to_dict(orient='records')
    insert_direct(data_list, tablename=tablename)
